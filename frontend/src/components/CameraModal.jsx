...
const CameraModal = ({ isOpen, onClose, onSave }) => {
    ...
    const captureFrame = async () => {
        ...
        try {
            const response = await fetch('http://backend:5000/process_frame', { // Change the URL to match Docker service name
                method: 'POST',
    ...
    const sendCommand = async (command) => {
        try {
            await fetch(`http://backend:5000/${command}`, { // Change the URL to match Docker service name
                method: 'POST',
            });
        } catch (error) {
    ...
    const saveSignature = async () => {
        ...
        canvasRef.current.toBlob(async (blob) => {
            ...
            try {
                const response = await fetch('http://backend:5000/save_signature', { // Change the URL to match Docker service name
                    method: 'POST',
                    body: formData,
                });
